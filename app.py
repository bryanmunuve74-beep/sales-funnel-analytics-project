...
if len(purchases_df) > 0:
    ...
    if len(purchases_df) > 10:
        ...
        st.dataframe(
            ...
            .apply(css_background_gradient, subset=["Revenue"],
                   color_low=GRADIENT_COLORS["RdYlGn"][0], color_high=GRADIENT_COLORS["RdYlGn"][1]),
...
if len(rep_perf) > 0:
    ...
    st.dataframe(
        ...
        .apply(css_background_gradient, subset=["Revenue"],
               color_low=GRADIENT_COLORS["Blues"][0], color_high=GRADIENT_COLORS["Blues"][1]),
        use_container_width=True
    )
...
if len(ch_analysis) > 0:
    ...
    st.dataframe(
        ...
        .apply(css_background_gradient, subset=["Revenue"],
               color_low=GRADIENT_COLORS["Greens"][0], color_high=GRADIENT_COLORS["Greens"][1]),
        use_container_width=True
    )
...
if len(country_perf) > 0:
    ...
    st.dataframe(
        country_perf.style.format({
            "Revenue": "${:,.0f}",
            "Deals": "{:.0f}",
            "Unique_Customers": "{:.0f}"
        })
        .apply(css_background_gradient, subset=["Revenue"],
               color_low=GRADIENT_COLORS["Purples"][0], color_high=GRADIENT_COLORS["Purples"][1]),
        use_container_width=True
    )
else:
...
if len(purchases_df) > 0:
    ...
    st.dataframe(
        disc_impact.style.format({
            "Count": "{:.0f}",
            "Revenue": "${:,.0f}",
            "Avg Discount": "${:.2f}"
        })
        .apply(css_background_gradient, subset=["Revenue"],
               color_low=GRADIENT_COLORS["Reds"][0], color_high=GRADIENT_COLORS["Reds"][1]),
        use_container_width=True,
        hide_index=True
    )
else:
